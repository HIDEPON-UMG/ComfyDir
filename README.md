...
| **永続 UI 状態** | 直近フォルダ／サイズ／ソート／フィルタ／検索／ペイン幅を `localStorage` に保存 |
...
6. 上部の「プロンプト内検索」にキーワード（空白区切りで AND）
...
| `GET` | `/api/images` | 一覧（クエリ: `folder_id`, `tags=a,b`, `tag_mode=and\|or`, `order=name\|mtime\|added`, `direction=asc\|desc`, `q=keyword`） |
