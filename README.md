...
| **監視フォルダ管理** | 「+ 追加」「↻ 再スキャン」「サブ含む／直下のみ トグル」「✎ 編集（パス／ラベル）」「解除」をヘッダーから直接操作。フォルダ追加/編集ダイアログには「参照…」ボタン (OS ネイティブのフォルダ選択ダイアログ呼出し) を内蔵。パス変更時は配下画像レコードのパス列も追従して書き換え、watchdog を新パスで再起動 |
| **再帰スキャンの切替** | フォルダごとに「サブディレクトリも含めて再帰スキャン」を ON/OFF できる。OFF 化時もサブフォルダ配下の DB レコード・Myタグ・メモは保持されるので、後で ON に戻せば即時復活する (削除は実ディスクから消えたファイルだけ) |
| **再スキャン進捗バー** | 再スキャン中はツールバー直下に進捗バーが表示。ファイル取込フェーズで `done/total` から計算した width が左→右へ単調に伸びる。列挙・後始末中はボタンアイコン回転のみで表現 |
...
このフォールバック画面には **「ComfyDir を起動」ボタン** があり、`start.vbs` をダブルクリックしに行かなくてもその場でサーバを立ち上げ直せます。ブラウザのサンドボックスはローカルの `.vbs` を直接実行できないため、`launcher.py` が起動時に `HKEY_CURRENT_USER` へ登録するカスタム URL プロトコル `comfydir://` を経由して OS 側に `start.vbs` を起動させる仕組みです（管理者権限不要・登録は冪等）。ボタン押下後はバックエンドの復帰を自動ポーリングし、立ち上がり次第アプリ画面へ遷移します。

> **補足**: プロトコルは「過去に一度でも正常起動できた = launcher が走った」状態で登録されるため、**初回の一度だけは従来どおり `start.vbs` をダブルクリック**してください。それ以降はこのボタンで起動できます。また Chrome は初回クリック時に「`comfydir://` を開きますか？」という確認ダイアログを 1 回だけ表示します（「常に許可」で以降は抑制）。
...
| `POST` | `/api/folders/{id}/rescan` | 手動再スキャン (進捗は SSE `scan_progress` で配信) |
| `POST` | `/api/pick-dir` | OS ネイティブのフォルダ選択ダイアログを開いて選択された絶対パスを返す (Tkinter を別 subprocess で起動、UTF-8 強制で日本語パス透過) |
...
| `CIO_PORT` | `8772` | バインドポート（解決順位は下記参照） |
| `PROJECTFOLDERS_PORTS_FILE` | `%LOCALAPPDATA%\ProjectFolders\ports.json` | 共有ポートレジストリの場所を上書き |

### ポートの決まり方（共有レジストリ）

ProjectFolders 配下の複数ローカルアプリ（ComfyDir / CCA-StudyApp / notebooklm-mcp）が
同じポートを既定値にして奪い合う事故（2026-06-02: ComfyDir の画面に CCA の Basic 認証
ダイアログが出た件）の再発防止として、ポートは共有レジストリ
`%LOCALAPPDATA%\ProjectFolders\ports.json` の `comfydir` キーで一元管理します。

解決順位は **環境変数 `CIO_PORT` > レジストリの `comfydir` > コード内既定値 `8772`**。
レジストリが無くても各アプリの既定値は互いに重複しないため衝突しません
（実装は [`port_registry.py`](src/comfy_image_organizer/port_registry.py)）。

```json
{ "ports": { "notebooklm-mcp": 8765, "cca-pwa": 8770, "cca-mcp": 8771, "comfydir": 8772 } }
```

> ポートを変更したら、ブラウザ側は新しい origin（例: `http://127.0.0.1:8772`）で
> PWA を入れ直してください（PWA のインストールは origin = ポートに紐づくため）。
...
ブラウザ側に `offline.html`（「ComfyDir サーバに接続できません」画面）が出ているだけなら、その画面の **「ComfyDir を起動」ボタン**で立ち上げ直せます（[オフライン挙動](#オフライン挙動) 参照）。ボタンが効かない／表示されない場合は、`comfydir://` プロトコル未登録（= まだ一度も正常起動していない）か、Service Worker が旧版キャッシュを配信している可能性があります。前者は `start.vbs` を一度ダブルクリック、後者はサーバ起動後に一度リロードして SW を更新してください。
